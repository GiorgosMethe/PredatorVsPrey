...
At this point we should note that we were only able to run this simulation for 1,2 and 3 predators (2,3 and 4 agents). Trying to run for 4 predators and one prey would require a huge amount of memory space, because of the billions of possible states of the world. There are $11^{10}$ (5 different pairs of $(x,y)$ coordinates) different states for which we need to calculate the Q table and each agent has 5 possible actions in each one of these states. So, initializing $5 \times 11^{10}$ action - value pairs would need $5 \times 11^{10} \times 16$ bytes, which is, $\approx 415$ Gigabytes of memory.
\begin{itemize}
