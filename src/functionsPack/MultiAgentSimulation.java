...
public class MultiAgentSimulation {
	...
	public static void addPredator(ArrayList<Coordinate> list, Vector<Agent> ws) {
		...
		Predator pred = new Predator("pred", new Coordinate(
				(int) Math.abs(10 * Math.random()), (int) Math.abs(10 * Math
						.random())), null);
	...
	public static void runSimulation(int no) {
		...
		System.out.println("Collisions:  " + collisions + "   Catches:   "
				+ catches);

		System.out.println("Average catch steps  :   " + sumCatchSteps
				/ catches);
	}
