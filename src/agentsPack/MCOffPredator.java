...
public class MCOffPredator extends LearningPredator { 
	...
	public void updateQTable(Vector<SARSdata> episode) {
		...
		for (SARSdata sars : episode) {
			if (sars.action.equals(this.pi.chooseAction(sars.currentState, this.qTable))) {
				break;
			}
			tau++;
		}
		...
		for (SARSdata sars : sublist) {
			w *= 1 / this.pi.actionProbability(sars.currentState, sars.action, qTable);
			...
			this.qTable.put(s_a, this.numerator.get(s_a) / this.denominator.get(s_a));
		}
	}
}
