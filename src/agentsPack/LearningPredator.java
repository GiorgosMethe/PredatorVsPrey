...
public abstract class LearningPredator extends Predator {
	...
	// In the implementing class, make sure this returns True if and only if the
	// algorithm is on-policy
	public abstract boolean onPolicyLearning();

	// Receive one state-action pair and its observed reward and next state per
	// update, and learn from the data.
	// If the algorithm is off-policy, implement the
	// updateQTable(Vector<SARSdata>).
	public void updateQTable(SARSdata sars) {
		throw new UnsupportedOperationException(
				"This is an off-policy learning algorithm.");
	}
	...
	public void updateQTable(Vector<SARSdata> episode) {
		throw new UnsupportedOperationException(
				"This is an on-policy learning algorithm.");
	}
	...
	public void observe(Vector<Agent> currentState, Coordinate action,
			Vector<Agent> nextState, Double reward) {
		this.observe(new SARSdata(currentState, action, nextState, reward));
	}

	public void observe(Vector<Agent> currentState, Coordinate action,
			Vector<Agent> nextState, Double reward, boolean isAbsorbingState) {
		this.observe(new SARSdata(currentState, action, nextState, reward,
				isAbsorbingState));
	}
